...
def make(env, target):
    ...
    if not isfile(join(target.builds.FREETYPE, 'config.mk')):
        ...
        cmd = './configure --enable-silent-rules LDFLAGS="-static-libgcc" --without-bzip2  --build=i686-pc-linux-gnu --host=x86_64-w64-mingw32 --disable-shared --enable-static --prefix="%s"'% (target.dist,)
