...
class preprocessor:
    ...
    def eval_defines(self, define):
        while True:
            try:
                ret = eval(define, self.build_eval_globals())
                break
            except NameError, ex:
                missing_name = str(ex).split("'")[1]
                self.undefines.append(missing_name)
        return ret
    ...
    # return values are (squelch, metadata)
        ...
        if line[:6] == '#ifdef' or line[:3] == '#if':
            try:
                self.__ifcondition = ' '.join(line.split(' ')[1:])
            except:
                self.exit_error('#ifdef')

            self.__ifblock = True
            self.__ifconditions.append(self.__ifcondition)
        ...
        if self.__ifblock is True:
            ...
            if line[:6] == '#ifdef' or line[:3] == '#if':
                self.__evalsquelch = not self.eval_defines(self.__ifcondition)
                return False, True
            # evaluate and process the #else
            elif line[:5] == '#else':
                self.__evalsquelch = self.eval_defines(self.__ifcondition)
                return False, True
            else:
    ...
    def parse(self):
        ...
        self.__inputBuffer = input_file.read()
        input_file.close()
        input_file = open(os.path.join(self.input),'r')
        self.__outputBuffer = ''
        try:
